...
import { writeOAuthTokenRecord } from "@/lib/oauth-token-store";

async function persistOAuthToken(token: {
  access_token: string;
  ...
  try {
    ...
    console.error("Failed to persist OAuth token for worker", error);
  }
}
